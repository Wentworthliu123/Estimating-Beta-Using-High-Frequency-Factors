...
steponeout='/project2/dachxiu/hf_ff_project/Implmt_Code/Xinyu_test/outlier/Stepone'
steptwoout='/project2/dachxiu/hf_ff_project/Implmt_Code/Xinyu_test/outlier/Steptwo'
stepthreeout='/project2/dachxiu/hf_ff_project/Implmt_Code/Xinyu_test/outlier/Stepthree'
stepfourout='/project2/dachxiu/hf_ff_project/Implmt_Code/Xinyu_test/outlier/Stepfour'
...
#         out_to_save_one = os.path.join(steponeout, "Step1_"+pdstart+'_'+pdend[4:]+"_intraday.csv")
#         DataPcnew.to_csv(out_to_save_one)
...
#         out_to_save_two = os.path.join(steptwoout, "Step2_"+pdstart+'_'+pdend[4:]+"_intraday.csv")
#         kd_m_p5m.to_csv(out_to_save_two)
        ...
        #Clean nomatch
        ret_kd_m_p5m=ret_kd_m_p5m.dropna(axis=0,subset=['symbol'])
        ...
        ####
        # Save step three result
        ####
#         out_to_save_three = os.path.join(stepthreeout, "Step3_"+pdstart+'_'+pdend[4:]+"_intraday.csv")
#         ret_kd_m_p5m.to_csv(out_to_save_three)
        step3 = ret_kd_m_p5m[['date', 'permno', 'retadj', 'wt', 'me']]
        step3.to_csv(os.path.join(TEMP_DIR, 'pre_step3_%s_%d.csv' % (year, k)))
...
#         out_to_save_four = os.path.join(stepfourout, "Step4_"+pdstart+'_'+pdend[4:]+"_intraday.csv")
        ff_factors_merged.to_csv(os.path.join(TEMP_DIR, 'pre_step4_%s_%d.csv' % (year, k)))
...
step3_list = list(filter(lambda x: x.endswith('.csv') and x.startswith('pre_step3'), step3_list))
step3_list.sort()
...
step3_df=step3_df.drop_duplicates()
...
step4_list = list(filter(lambda x: x.endswith('.csv') and x.startswith('pre_step4'), step4_list))
step4_list.sort()
...
step4_df=step4_df.drop_duplicates(subset = ['time'])
